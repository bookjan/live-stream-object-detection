...
const modelPath = path.resolve(
  __dirname,
  "/Users/jansu/Downloads/frozen_east_text_detection.pb"
);

if (!fs.existsSync(modelPath)) {
  console.log("could not find EAST model");
  console.log(
    "download the model from: https://github.com/oyyd/frozen_east_text_detection.pb/blob/71415464412c55bb1d135fcdeda498e29a67effa/frozen_east_text_detection.pb?raw=true" +
...
function decode(scores, geometry, confThreshold) {
  ...
  for (let y = 0; y < numRows; y += 1) {
    for (let x = 0; x < numCols; x += 1) {
      ...
      const endX =
        offsetX +
        cos * geometry.at([0, 1, y, x]) +
        sin * geometry.at([0, 2, y, x]);
      const endY =
        offsetY -
        sin * geometry.at([0, 1, y, x]) +
        cos * geometry.at([0, 2, y, x]);
      const startX = endX - w;
      const startY = endY - h;
...
function textDetection(img) {
  ...
  const inputBlob = cv.blobFromImage(
    ...
    new cv.Vec3(123.68, 116.78, 103.94),
    true,
    false
  );
  ...
  // write the jpg binary data
  process.send(cv.imencode(".jpg", img).toString("binary"));
}

const runWebcamObjectDetection = (src, textDetection) =>
  grabFrames(src, camInterval, frame => {
    const frameResized = frame.resizeToMax(opencv.frameSize);

    // detect objects
    textDetection(frameResized);
  });

runWebcamObjectDetection(opencv.camPort, textDetection);
